...
def main [host: string, currentSystem: string, cleanFlake: string, hostData: string] {
    ...
    if $runtime.local {
        ...
        log info $'(ansi blue_bold)>>>(ansi reset) ssh -t ($sshTarget) nix --extra-experimental-features '"nix-command flakes"' run ($nixArgs | str join) $"($cleanFlake)#activate-v2" host ($runtime.host)'
        ssh -t $sshTarget nix --extra-experimental-features '"nix-command flakes"' run ...$nixArgs $"($cleanFlake)#activate-v2 host ($runtime.host)"
    }
}
